...
for (i in 6:ncol(data_num_all_b6)){ # skip first 5 columns of tibble
  plots[[i]] <- tibble::tibble(mouse_id = data_num_all_b6$Mouse, 
    ...
    ggplot() + geom_boxplot(aes(y = trait_value, x = vaccination_status, colour = line )) + facet_wrap(~ experiment) + ggtitle(colnames(data_num_all_b6)[i])
}
```

```{r}
ggpubr::ggarrange(plotlist = plots[-c(1:5)], ncol = 1)
```
